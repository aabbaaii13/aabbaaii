...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Fypaf'.", Duration = 7, Image = 4400697855})
		else
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "FYPAF Notification",
					Content = "Enjoying This Script? Please Speard To People.",
					Duration = 7,
					Image = 4370033185,
				})
			end
